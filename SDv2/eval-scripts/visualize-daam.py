...
def generate_images(model_name, prompts_path, save_path, device='cuda:0', guidance_scale = 7.5, image_size=512, ddim_steps=100, num_samples=10, from_case=0):
    ...
    if model_name not in ['SD-v1-4','SD-V2','SD-V2-1']:
        try:
            if '.pt' in model_name:
                model_name = model_name.replace('.pt','')
                unet.load_state_dict(torch.load(f'models/{model_name}.pt'))
            else:
    ...
    for _, row in df.iterrows():
        ...
        for num, im in enumerate(pil_images):
            im.save(f"{folder_path}/{prompts_name}/{case_number}_{num}.png")
