...
def generate_images_jump(model_name, prompts_path, save_path, device='cuda:0', guidance_scale = 7.5, image_size=512, ddim_steps=100, num_samples=10, from_case=0):
    '''
    ...
    prompts_name = prompts_path.split('/')[-1].split('.')[0]

    folder_path = f'{save_path}/{model_name}'
    os.makedirs(folder_path, exist_ok=True)
    os.makedirs(f'{folder_path}/{prompts_name}', exist_ok=True)
    ...
    for _, row in df.iterrows():
        ...
        stop_timestep = np.random.randint(1, num_inference_steps)
        ...
        for t in tqdm(scheduler.timesteps):
            ...
            if count_timestep > stop_timestep:
                ...
                print("Something went wrong, the stop timestep should be smaller than the current timestep", count_timestep, t, stop_timestep, num_inference_steps)
                exit()
        ...
        for num, im in enumerate(pil_images):
            im.save(f"{folder_path}/{prompts_name}/{case_number}_{num}_{stop_timestep}.png")
...
if __name__=='__main__':
    ...
    generate_images_jump(model_name, prompts_path, save_path, device=device,
