...
from utils_alg import load_model_from_config, load_model_from_config_compvis
...
def main(opt):
    ...
    try:
        ...
        print("Loading model from config failed, trying compvis model")
        model = load_model_from_config_compvis(config, f"{opt.ckpt}", verbose=True)
    ...
    if not opt.prompts_path:
        prompt = opt.prompt
        assert prompt is not None
        data = [batch_size * [prompt]]

    else:
        print(f"reading prompts from {opt.prompts_path}")

        df = pd.read_csv(opt.prompts_path)
        data = []
    ...
    if opt.torchscript or opt.ipex:
        ...
        with torch.no_grad(), additional_context:
            for _ in range(1): # CHANGE THIS TO 1
    ...
    assert opt.n_iter == 1, "n_iter must be 1 for now"
    ...
    with torch.no_grad(), \
        ...
        model.ema_scope():
            for i, prompts in enumerate(data):
                if i < opt.from_case:
                    continue
                if i > opt.to_case and opt.to_case != -1:
                    break
