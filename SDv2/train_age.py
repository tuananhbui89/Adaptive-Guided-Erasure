...
def train_prompt(prompt, train_method, start_guidance, negative_guidance, iterations, lr, config_path, ckpt_path, diffusers_config_path, devices, seperator=None, image_size=512, ddim_steps=50, args=None):
    '''
    ...
    if not os.path.exists('models/embedding_matrix_dict_EN3K.pt'):
        save_embedding_matrix(model, model_name='SD-v2-1', save_mode='dict', vocab='EN3K')

    if not os.path.exists('models/embedding_matrix_array_EN3K.pt'):
        save_embedding_matrix(model, model_name='SD-v2-1', save_mode='array', vocab='EN3K')
    
    if not os.path.exists('models/embedding_matrix_array_Imagenet.pt'):
        save_embedding_matrix(model, model_name='SD-v2-1', save_mode='array', vocab='Imagenet')
    ...
    for word in erased_words:
        if args.vocab in ['EN3K', 'Imagenet', 'CLIP']:
            # search closest tokens from the vocab, using the similarity matrix
            vocab = args.vocab
        ...
        top_k_tokens, sorted_sim_dict = search_closest_tokens(word, model, k=args.gumbel_k_closest, sim='l2', model_name='SD-v2-1', ignore_special_tokens=args.ignore_special_tokens, vocab=vocab)
    ...
    for erase_word in erased_words:
        ...
        # preserved_matrix = torch.cat([create_prompt(word) for word in preserved_set], dim=0) # [n, 77, 1024]
        preserved_matrix = preserved_matrix.flatten(start_dim=1) # [n, 77*1024]
    ...
    """
    v6: change the gumbel softmax, if `hard` is 1, then it is one-hot, if `hard` is 0, then it is a new soft version, which takes the top-k highest values and normalize them to 1
    """
    ...
    for i in pbar:
        ...
        # get the emb_r 
        emb_r = torch.reshape(torch.matmul(gumbel_softmax(one_hot_dict[word]), preserved_matrix_dict[word]).unsqueeze(0), (1, 77, 1024))
        assert emb_r.shape == emb_n.shape

        # clone the emb_r for the time step
        emb_0 = emb_r.clone().detach()
        ...
        if i % args.pgd_num_steps == 0:
            ...
            loss += args.lamda * criteria(z_r_wo_prompt_pred.to(devices[0]), z_r_org_pred.to(devices[0])) # for preserved concepts, output are the same without prompt
            ...
            # one_hot.grad = None
            loss = 0 
            loss -= criteria(z_n_wo_prompt_pred.to(devices[0]), z_0_org_pred.to(devices[0]))
            loss -= args.lamda * criteria(z_r_wo_prompt_pred.to(devices[0]), z_r_org_pred.to(devices[0])) # maximize the preserved loss
            ...
            history_dict = save_to_dict(one_hot_dict, f'one_hot_dict_{i}', history_dict)
        ...
        if i % (args.save_freq) == 0:
            with torch.no_grad():
                for word in erased_words:
                    ...
                    emb_r_eval = torch.reshape(torch.matmul(gumbel_softmax(one_hot_dict[word]), preserved_matrix_dict[word]).unsqueeze(0), (1, 77, 1024))
