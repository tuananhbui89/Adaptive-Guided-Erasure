...
def main():
    ...
    if args.model_name == 'SD-v1-4':
        ...
        assert args.ckpt_path, 'Please provide ckpt path for stable diffusion v1-4'
        model = load_model_from_config(config, args.ckpt_path)
    elif args.model_name == 'SD-v2-1': 
        print('Using stable diffusion v2-1 model')
        assert args.ckpt_path, 'Please provide ckpt path for stable diffusion v2-1'
        model = load_model_from_config(config, args.ckpt_path)
    else:
        assert not args.ckpt_path, 'Loading from pre-trained model, please provide model name only.'
