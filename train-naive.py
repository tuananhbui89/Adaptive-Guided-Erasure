...
def train_esd(prompt, train_method, start_guidance, negative_guidance, iterations, lr, config_path, ckpt_path, diffusers_config_path, devices, seperator=None, image_size=512, ddim_steps=50, info=''):
    ...
    target_print = args.target.replace(' ','')

    prompt, _ = get_prompt(prompt)
    ...
    name = f'compvis-naive-word_{word_print}-target_{target_print}-method_{train_method}-sg_{start_guidance}-ng_{negative_guidance}-iter_{iterations}-lr_{lr}-info_{info}'
    # TRAINING CODE
    ...
    for i in pbar:
        ...
        emb_0 = model.get_learned_conditioning([args.target]) # CHANGE HERE
        emb_p = model.get_learned_conditioning([word])
        emb_n = model.get_learned_conditioning([f'{word}'])
        emb_preserved = model.get_learned_conditioning([' ']) # CHANGE HERE, preserve empty string
        ...
        with torch.no_grad():
            ...
            e_preserved_org = model_orig.apply_model(z.to(devices[1]), t_enc_ddpm.to(devices[1]), emb_preserved.to(devices[1]))


        # breakpoint()
        # get conditional score from ESD model
        e_n = model.apply_model(z.to(devices[0]), t_enc_ddpm.to(devices[0]), emb_n.to(devices[0]))
        e_preserved = model.apply_model(z.to(devices[0]), t_enc_ddpm.to(devices[0]), emb_preserved.to(devices[0]))
        ...
        loss += args.lamda * criteria(e_preserved.to(devices[0]), e_preserved_org.to(devices[0]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target', help='target concept to map to', type=str, required=False, default='')
    parser.add_argument('--lamda', help='tradeoff parameter', type=float, required=False, default=0.1)
