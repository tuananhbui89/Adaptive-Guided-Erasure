...
def main(args):
    ...
    if args.multi_concept is not None:
        ...
        print(args.multi_concept) # [['Cassette Player', 'object']] or [[['Cassette Player', 'object'], ['Church', 'object'], ['Garbage Truck', 'object'], ['Parachute', 'object'], ['French Horn', 'object']]]
        print(args.multi_concept[0]) # ['Cassette Player', 'object'] or [['Cassette Player', 'object'], ['Church', 'object'], ['Garbage Truck', 'object'], ['Parachute', 'object'], ['French Horn', 'object']]
        if len(args.multi_concept[0]) == 2 and len(args.multi_concept) == 1:
            args.multi_concept = [[args.multi_concept[0][0], args.multi_concept[0][1], -1]]
        elif len(args.multi_concept[0]) > 2:
            args.multi_concept = [[c, t, -1] for c, t in args.multi_concept[0]]
        else: 
            raise ValueError("multi_concept format is not correct")   
        
        print(args.multi_concept)

        for c, t, _ in args.multi_concept:
            c = c.replace('-', ' ')
            if t == "object":
    ...
    torch.manual_seed(1)
    output_folder = f"evaluation_folder/{args.pretrained_model_name_or_path}/generated_images"
    os.makedirs(output_folder, exist_ok=True)

    for prompt in prompts:
        print(f'Inferencing: {prompt}')
        images = pipe(prompt, num_inference_steps=50, guidance_scale=7, num_images_per_prompt=5).images
